	...
	<main>
		<div class="nav-header">
			<div class="nav-header-links">
				<a href="<%=request.getContextPath()%>/list">[トップページ]</a> <a
					href="<%=request.getContextPath()%>/admin/inquiries">[お問い合わせ一覧]</a>
			</div>
		...
		<h3 style="text-align: center; margin-top: 30px;">管理者からの返信</h3>
			...
			<% if (inquiry.getCompatibilityStatus() == 0) { %>
				...
				<form action="<%=request.getContextPath()%>/admin/detailInquiry" method="post">
					...
					<table  style="border-collapse: collapse;　text-align:center; border:1;">
						<tr>
							<th width="150" style="background-color: #e0e0e0; text-align: right; vertical-align: top; padding: 5px;">返信内容：</th>
							<td width="350" style="padding: 5px;">
								<textarea name="replyComment" rows="5" style="width: 100%; resize: none;"></textarea>
							</td>
						</tr>
					</table>
					<div style="text-align: center; margin-top: 15px;">
						<input type="submit" value="返信を送信する" onclick="return confirm('この内容で返信を登録しますか？');">
					</div>
				</form>
			<% } else { %>
				...
				<table  style="border-collapse: collapse;　text-align:center; border:1;">
					<tr>
						<th width="150" style="background-color: #e0e0e0; text-align: right; vertical-align: top; padding: 5px;">返信日時：</th>
					...
					<tr>
						<th style="background-color: #e0e0e0; text-align: right; vertical-align: top; padding: 5px;">返信内容：</th>
						<td style="padding: 5px; height: 80px; vertical-align: top;">
							<%=inquiry.getAdministratorReply() != null ? inquiry.getAdministratorReply().replace("\n", "<br>") : ""%>
						</td>
					</tr>
				</table>
